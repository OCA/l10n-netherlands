...
class cbs_export_file(models.Model):
    ...
    def set_invoice(self):
        ...
        if not invoices:
            ...
            invoices.search([('cbs_export_id', '=', self.id)]).write(
                {'cbs_export_id': False}
            )
    ...
    def cron_get_cbs_export_file(self):
        ...
        if not cbs_export_file:
            ...
            cbs_export_file.message_post(
                body=_("CBS Export is created for month %s in year %s") % (calendar.month_name[int(cbs_export_file.month)], cbs_export_file.year),
                subtype='mt_comment'
            )
    ...
    def export_file(self):
        ...
        cbs_export_data = \
            str('9801') + \
            str(company_id.vat or '').replace(' ', '')[2:].ljust(12) + \
            str(datetime.now().strftime("%Y%m").ljust(6)) + \
            str(company_id.name or '').ljust(40) + \
            str(" " * 6) + \
            str(" " * 5) + \
            str(datetime.now().strftime("%Y%m%d").ljust(8)) + \
            str(datetime.now().strftime("%H%M%S").ljust(6)) + \
            str(company_id.phone or '').replace(' ', '')[0:15].ljust(15) + \
            str(" " * 13) + \
            '\n'
        for invoice_line in self.account_invoice_ids.mapped('invoice_line_ids'):
            ...
            if len(str(invoice_line.invoice_id.number or '')) < 8:
                ...
                )[:8].ljust(8) + str(line_number).zfill(2)
            value += str(" " * 3) + \
                    str(" " * 1) + \
                    str('000') + \
                    str(" " * 7) + \
