...
class KvKPreviewWizard(models.TransientModel):
    ...
    line_ids = fields.One2many("l10n.nl.kvk.preview.wizard.line", "wizard_id", "Lines")

    @api.model
    def _get_wizard_lines_values(self, target_item):
        def _get_partner_name(target_item):
            trade_names = target_item["tradeNames"]
            partner_name = trade_names.get("businessName") or trade_names.get(
                "shortBusinessName"
            )
            return partner_name
        ...
        def _get_country(first_address):
            ...
            if first_address.get("country") == "Nederland":
                ...
                country = self.env["res.country"].search(
                    [("name", "ilike", first_address["country"])], limit=1
                )
            return country
...
class KvKPreviewWizardLines(models.TransientModel):
    ...
    entity_type = fields.Selection(
        [("legal_person", "Legal Person"), ("headquarters", "Headquarters")]
    )
